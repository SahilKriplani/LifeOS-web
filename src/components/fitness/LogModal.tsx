...
import { useCreateFitnessLog } from "@/hooks/useFitness";
import {
  useExercises,
  useCreateExercise,
  useCreateWorkout,
...
type LogTab = "body" | "workout";

interface LogModalProps {
  open: boolean;
  onClose: () => void;
  /** Which tab to show when the modal opens. Defaults to "body". */
  initialTab?: LogTab;
}
...
type BodyForm = yup.InferType<typeof bodySchema>;

function BodyTab({ onDone }: { onDone: () => void }) {
  ...
  const {
    ...
    resolver: yupResolver(bodySchema) as any,
    defaultValues: { weightKg: undefined, calories: 0, steps: 0 },
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <DatePicker value={logDate} onChange={setLogDate} max={getTodayISO()} />
      </div>
...
function WorkoutTab({ onDone }: { onDone: () => void }) {
    ...
    const payloadEntries = entries
      .map((e) => ({
        ...
        sets: e.sets
          ...
          .map((s) => ({
            weightKg:
              s.weight.trim() === "" ? null : toKg(parseFloat(s.weight), unit),
...
export default function LogModal({
  ...
  const tabs: { key: LogTab; label: string; icon: typeof Scale }[] = [
        ...
        <div className="p-6 flex flex-col gap-5 overflow-y-auto min-h-0">
          <DialogHeader>
            ...
            <DialogDescription style={{ color: "var(--muted-foreground)" }}>
              Track your body metrics or log a workout — all in one place.
            </DialogDescription>
          </DialogHeader>
          ...
          {/* Both tabs stay mounted so in-progress input survives a tab switch. */}
          <div className={tab === "body" ? "" : "hidden"}>
            <BodyTab onDone={onClose} />
          </div>
          <div className={tab === "workout" ? "" : "hidden"}>
            <WorkoutTab onDone={onClose} />
          </div>
        </div>
      </DialogContent>
    </Dialog>
